...
def object_detection_collate(batch):
    ...
    for image, boxes, labels in batch:
        if image_type is np.ndarray:
            ...
            raise TypeError(
                f"Image should be tensor or np.ndarray, but got {image_type}.")
        if box_type is np.ndarray:
            ...
            raise TypeError(
                f"Boxes should be tensor or np.ndarray, but got {box_type}.")
        if label_type is np.ndarray:
            ...
            raise TypeError(
                f"Labels should be tensor or np.ndarray, but got {label_type}.")
